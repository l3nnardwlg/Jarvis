...
function streamOllamaChat(messages, options = {}) {
  const endpointUrl = new URL(OLLAMA_CHAT_ENDPOINT);
  const client = endpointUrl.protocol === "https:" ? https : http;
  ...
  return new Promise((resolve, reject) => {
    const request = client.request(
      ...
      (response) => {
        if (response.statusCode < 200 || response.statusCode >= 300) {
          reject(new Error(`Ollama Chat HTTP ${response.statusCode}`));
          return;
        }
    ...
    request.on("timeout", () => {
      request.destroy(new Error("Ollama Chat Anfrage lief in ein Timeout."));
    });
