...
function buildFollowUpMathExpression(input, lastResult) {
  ...
  const wordTransforms = [
    ...
    [/^plus\s+/i, "+ "],
    [/^minus\s+/i, "- "],
    [/^(geteilt\s+durch|durch)\s+/i, "/ "],
    [/^modulo\s+/i, "% "],
  ];

  let expressionSuffix = normalizedInput;
  wordTransforms.forEach(([pattern, replacement]) => {
    expressionSuffix = expressionSuffix.replace(pattern, replacement);
  });

  if (/^[+\-*/%]\s*\d/.test(expressionSuffix)) {
    return `${lastResult} ${expressionSuffix}`;
  }
